...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	Cromossoma aux,hill,best;
	static Random a;
	int nMutacoes,nCrossovers,nTorneios,nGladio,tamanhoPop,tamanhoStr,bprint,nInv;
	unsigned long long geracoes;
	double pMutacoes,pmMuta,sdMuta,pCrossovers,pmCrossover,
	       sdCrossover,pCrossMutaConsang,pCrossMutaConsangInv,pInv;
	...
	if(nrhs < 16){
		pCrossMutaConsangInv = 0.9; 
	} else {
		verificaIntervalo<double>(nrhs,prhs,pCrossMutaConsangInv,15,0);
		//mexPrintf("%f\n",pCrossMutaConsang);
	}
	if(nrhs < 17){
		nInv = 5; 
	} else {
		verificaIntervalo<int>(nrhs,prhs,nInv,16,0);
		//mexPrintf("%f\n",pCrossMutaConsang);
	}
	...
	if(nrhs < 18){
		...
		verificaIntervalo<double>(nrhs,prhs,pInv,17,0);
		//mexPrintf("%f\n",pCrossMutaConsang);
	}
	...
	if(nrhs < 19){
		...
		verificaIntervalo<int>(nrhs,prhs,bprint,18,1);
	}
	...
	for(int j=0;j<geracoes;j++){
		...
		pop.torneio(nTorneios,nGladio); //alteração
		for(int i=0;i<nCrossovers;i++) if(a.flip(pCrossovers)) pop.crossover(pmCrossover,
																		 sdCrossover,pCrossMutaConsang,pCrossMutaConsangInv); //as prob vão ser arg de entrada
		for(int i=0;i<nMutacoes;i++)
			if(a.flip(pMutacoes)) pop.mutacao(pmMuta,sdMuta);
		
		
		//for(int i=0;i<nInv;i++)
		//	if(a.flip(pInv))	  pop.mutacaoInversa();
		...
		for(int i=0;i<aux.getDim();i++){
		  hill = aux;
		  hill.flip(i);
		  hill.calculaFitness(nrhs,(mxArray**)prhs);
		  if(hill.getFitness()<best.getFitness())
		    best = hill;
		}
		
		if(best.getFitness()<aux.getFitness())
		    pop.maiorFitness() = best;
		else
